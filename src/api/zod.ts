...
import type { ZodType, ZodTypeDef, string as zodString } from 'zod';

// just name it to be more verbose
type DecimalString = `${number}`;
type PositiveHexString = `0x${string}`;

export type { TypeOf } from 'zod';

// TODO: find way to extend zod
const createDecimalStringPrecisionValidator = (precision: number) => (data: string) => {
  const regex = new RegExp(`^\\d+(?:\\.\\d{0,${precision}})?$`);

  return regex.test(data);
};

const createDecimalStringZodScheme = (params?: Parameters<typeof zodString>[0]) =>
  z.string(params).regex(/^\d*\.?\d*$/g) as ZodType<DecimalString, ZodTypeDef, string>;
...
export const zod = {
  ...
  positiveHexString: (params?: Parameters<typeof zodString>[0]) =>
    z
      ...
      .regex(/^0x[0-9A-Fa-f]*$/) as ZodType<PositiveHexString, ZodTypeDef, string>,
